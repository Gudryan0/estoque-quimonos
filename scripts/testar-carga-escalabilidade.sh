...
CSV="evidencias/carga-escalabilidade-$DATA.csv"
RELATORIO="evidencias/carga-escalabilidade-$DATA.txt"
...
restaurar()
{
    ...
    if docker compose exec nginx nginx -t \
        >/dev/null 2>&1; then
...
executar_cenario()
{
    ...
    local nome="${quantidade} nó"

    if (( quantidade > 1 )); then
        nome="${quantidade} nós"
    fi

    nome="$nome Laravel - concorrência $concorrencia"
...
{
    echo "============================================================"
    echo " TESTE DE CARGA E ESCALABILIDADE"
    echo "============================================================"
    echo
    echo "Data:"
    ...
    echo
    echo "Requisições:"
    echo "  $REQUISICOES por repetição"
    echo
    echo "Repetições:"
    echo "  $REPETICOES"
    echo
